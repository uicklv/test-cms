                        ...
                        <?php if (isset($this->list) && is_array($this->list) && count($this->list)) { ?>
                            <?php foreach ($this->list as $item) { ?>
                                <tr id="item_<?= $item->id; ?>" class="tr-hovered">
                                    ...
                                    <td>
                                        <?php echo $item->current_company_name?>
                                    </td>
                                    <td>
                                        <?php echo '£' . numberFormatInStr($item->average_salary) ?>
                                    </td>
                                    <td>
                                        <?php echo '£' . numberFormatInStr($item->minimum_salary) ?>
                                    </td>
                                    <td>
                                        <?php echo '£' . numberFormatInStr($item->maximum_salary) ?>
                                    </td>
                                        ...
                                        <div class="items-row align-items-center">
                                            <div class="dropdown dropup custom-dropdown-icon">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink-3">
                                                    <a @click="load('panel/salary_survey/delete/<?= $item->id; ?>');" class="dropdown-item remove-item"><i class="fa fa-trash-alt"></i> Delete</a>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
