<?php
class Blog_codexController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $this->view->list = Blog_codexModel::getAll();
    ...
    public function filterAction()
    {
        ...
        $this->view->list = Blog_codexModel::getAllByCategoryId($category);
    ...
    public function paginationAction()
    {
        ...
        $totalRecords = Blog_codexModel::countBlogs();

        //total number of record with filtering
        $totalRecordWithFilter = Blog_codexModel::countBlogs($where);

        //search
        $data = Blog_codexModel::search($where, $orderby, $sort, $start, $end);
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if (!$result && $insertID) {
                    ...
                    Request::addResponse('redirect', false, url('panel', 'blog_codex', 'edit', $insertID));
                } else {
    ...
    public function editAction()
    {
        ...
        $this->view->blog = Blog_codexModel::get($id);

        if (!$this->view->blog)
            redirect(url('panel/blog_codex'));
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if ($this->view->blog->image !== $data['image']) {
                    if (File::copy('data/tmp/' . $data['image'], 'data/blog_codex/' . $data['image'])) {
                        File::remove('data/blog_codex/' . $this->view->blog->image);
                        File::remove('data/blog_codex/mini_' . $this->view->blog->image);
                        File::resize(
                            _SYSDIR_ . 'data/blog_codex/' . $data['image'],
                            _SYSDIR_ . 'data/blog_codex/mini_' . $data['image'],
                            358*2, 264*2
                        );
                    } else
    ...
    {
        $this->view->list = Blog_codexModel::getArchived();
    ...
    public function resumeAction()
    {
        ...
        if (!$user)
            redirect(url('panel/blog_codex/archive'));
    ...
    public function statisticAction()
    {
        ...
        $this->view->list = Blog_codexModel::getViewsByDays($this->view->blog->id);
        $this->view->referrals = Blog_codexModel::getReferrersList($this->view->blog->id);
        $this->view->views = Blog_codexModel::getViews($this->view->blog->id);
    ...
    {
        $id = (Request::getUri(0));
        $user = Blog_codexModel::getReferrer($id);
...
//            Request::addResponse('redirect', false, url('panel', 'blog', 'edit', $insertID));
        ...
        redirect(url('panel/blog_codex/statistic/' . $user->blog_id));
    }
}
/* End of file */
