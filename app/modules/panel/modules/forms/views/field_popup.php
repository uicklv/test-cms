<?php Popup::head('Add New Field'); ?>
    ...
    <form id="field_form" class="ppsec" method="post" enctype="multipart/form-data">
        <div class="form-row">
                    ...
                    <?php foreach ($this->fields as $item) { ?>
                        <?php if ($item->duplicate == 0) { ?>
                            <div class="custom-control custom-checkbox checkbox-info">
                                <input class="custom-control-input" type="checkbox" name="field_ids[]" id="field_<?=$item->id?>" value="<?= $item->id; ?>"
                                    <?= checkCheckboxValue(post('field_ids'), $item->id, stringToArray($this->section->fields_row)); ?>
                                ><label class="custom-control-label" for="field_<?=$item->id?>"><?= $item->title ?></label>
                            </div>
                            <?php } ?>
                        <?php } ?>
                    <?php } ?>
            ...
            <div class="form-group col-md-12">
                <a class="btn btn-success" href="" onclick="load('panel/forms/field_popup/<?= $this->section->id ?>/<?= $this->form->id ?>', 'form:#field_form', 'action_type=list'); return false;">
                    Add
                </a>
            </div>
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-12">
                <a class="btn btn-success" href="" onclick="setTextareaValue(); load('panel/forms/field_popup/<?= $this->section->id ?>/<?= $this->form->id ?>', 'form:#field_form', 'action_type=new'); return false;">
                    Add
                </a>
...
<?php Popup::foot(); ?>
    ...
    <script>
        ...
        $(document).ready(function() {
            if ($('#type').val() === 'dropdown' || $('#type').val() === 'checkbox') {
