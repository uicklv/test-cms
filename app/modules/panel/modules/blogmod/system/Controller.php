<?php
class BlogmodController extends Controller
{
    ...
    {
        $this->view->list = BlogmodModel::getAll();
    ...
    {
        Request::setTitle('Editor');
    }

    public function addAction()
    {
        Model::import('panel/blogmod/categories');
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if (!$result && $insertID) {
                    ...
                    Request::addResponse('redirect', false, url('panel', 'blogmod', 'edit', $insertID));
                } else {
    ...
    public function editAction()
    {
        ...
        $this->view->blog = BlogmodModel::get($id);

        if (!$this->view->blog)
            redirect(url('panel/blogmod'));
    ...
    {
        $this->view->list = BlogmodModel::getArchived();
    ...
    public function resumeAction()
    {
        ...
        if (!$user)
            redirect(url('panel/blogmod/archive'));
    ...
    public function statisticAction()
    {
        ...
        $this->view->list = BlogmodModel::getViewsByDays($this->view->blog->id);
        $this->view->referrals = BlogmodModel::getReferrersList($this->view->blog->id);
        $this->view->views = BlogmodModel::getViews($this->view->blog->id);
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if (!$result && $insertID) {
                    ...
                    Request::addResponse('redirect', false, url('panel', 'blogmod', 'statistic', $this->view->blog->id));
                } else {
                    Request::returnError('Database error');
                }
            } else {
    ...
    {
        $id = (Request::getUri(0));
        $user = BlogmodModel::getReferrer($id);
...
//            Request::addResponse('redirect', false, url('panel', 'blog', 'edit', $insertID));
        ...
        redirect(url('panel/blogmod/statistic/' . $user->blog_id));
    }
}
/* End of file */
