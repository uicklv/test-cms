<?php
class Blog_ckeditorController extends Controller
{
    ...
    {
        $this->view->list = Blog_ckeditorModel::getAll();
    ...
    {
        Model::import('panel/blog_ckeditor/categories');
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if (!$result && $insertID) {
                    ...
                    Request::addResponse('redirect', false, url('panel', 'blog_ckeditor', 'edit', $insertID));
                } else {
    ...
    public function editAction()
    {
        ...
        $this->view->blog = Blog_ckeditorModel::get($id);

        if (!$this->view->blog)
            redirect(url('panel/blog_ckeditor'));
    ...
    public function deleteAction()
    {
        ...
        $id = intval(Request::getUri(0));
        $user = Blog_ckeditorModel::get($id);
    ...
    {
        $this->view->list = Blog_ckeditorModel::getArchived();
    ...
    public function resumeAction()
    {
        ...
        if (!$user)
            redirect(url('panel/blog_ckeditor/archive'));
    ...
    public function statisticAction()
    {
        ...
        $this->view->list = Blog_ckeditorModel::getViewsByDays($this->view->blog->id);
        $this->view->referrals = Blog_ckeditorModel::getReferrersList($this->view->blog->id);
        $this->view->views = Blog_ckeditorModel::getViews($this->view->blog->id);
        ...
        if ($this->startValidation()) {
            ...
            if ($this->isValid()) {
                ...
                if (!$result && $insertID) {
                    ...
                    Request::addResponse('redirect', false, url('panel', 'blog_ckeditor', 'statistic', $this->view->blog->id));
                } else {
                    Request::returnError('Database error');
                }
            } else {
    ...
    {
        $id = (Request::getUri(0));
        $user = Blog_ckeditorModel::getReferrer($id);
...
//            Request::addResponse('redirect', false, url('panel', 'blog', 'edit', $insertID));
        ...
        redirect(url('panel/blog_ckeditor/statistic/' . $user->blog_id));
    }
}
/* End of file */
