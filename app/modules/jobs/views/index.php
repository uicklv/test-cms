...
<section class="section_15">
    <form id="search_form" class="fixed" method="post">
        <div class="search-jobs">
            <input id="keywords" name="keywords" class="sj-text-field" type="text" placeholder="Keyword or job title" value="<?= post('keywords', false, $this->keywords); ?>">
            ...
            <div class="fj-item">
                ...
                <select class="select" id="type" name="type">
                    ...
                    <!--<option value="temporary">Temporary</option>-->
                        <?php foreach (['permanent', 'contract'] as $item) {?>
                            <option value="<?= $item ?>" <?= getSession('jobs_type') == $item ? 'selected' : '' ?>><?= ucfirst($item) ?></option>
                        <?php } ?>
                </select>
            </div>
            <div class="fj-item">
                ...
                <select class="select" id="sector" name="sector">
                    ...
                    <?php if ($this->sectors) { ?>
                        <?php foreach ($this->sectors as $item) { ?>
                            <option value="<?= $item->id; ?>" <?= getSession('jobs_sector_id') == $item->id ? 'selected' : '' ?>><?= $item->name; ?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
            </div>
                ...
                <select class="select" id="location" name="location">
                    ...
                    <?php if ($this->locations) { ?>
                        <?php foreach ($this->locations as $item) { ?>
                            <option value="<?= $item->location_id; ?>" <?= getSession('jobs_location_id') == $item->location_id ? 'selected' : '' ?>><?= $item->name; ?></option>
                        <?php } ?>
                    <?php } ?>
                </select>
            </div>
