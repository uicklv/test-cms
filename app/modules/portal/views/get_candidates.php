...
<input type="hidden"  id="srt" name="srt">
<?php if (is_array($this->candidates) && count($this->candidates) > 0) { ?>
<h3 class="title-small"><?= $this->job->title ?></h3>
<div class="specialty-name-flex">
    <h3 class="specialty-name">Candidates</h3>
    <div class="sn-flex">
        <div class="sn-item">
            <span class="sn-status sn-new"></span>
            <a class="link-status" onclick="load('portal/sort_status', 'status=1', 'vacancy_id=<?= $this->job->id ?>', 'sort#sort', 'srt#srt'); return false;" style="text-decoration: none; cursor: pointer;">
            New
            </a>
        </div>
        <div class="sn-item">
            <span class="sn-status sn-process"></span>
            <a class="link-status" onclick="load('portal/sort_status', 'status=2', 'vacancy_id=<?= $this->job->id ?>', 'sort#sort', 'srt#srt'); return false;" style="text-decoration: none; cursor: pointer;">
            In Process
            </a>
        </div>
        <div class="sn-item">
            <span class="sn-status sn-rejected"></span>
            <a class="link-status" onclick="load('portal/sort_status', 'status=3', 'vacancy_id=<?= $this->job->id ?>', 'sort#sort', 'srt#srt'); return false;" style="text-decoration: none; cursor: pointer;">
                Rejected
            </a>
        </div>
    </div>
</div>
<div id="candidates_sort_box">
                ...
                <?php if ($item->cv) { ?>
                    <div>
                        ...
                        <a class="sb-download" href="<?= _SITEDIR_ ?>data/candidates/<?= $item->cv ?>"
                           download="<?= $item->firstname . '-' . $item->lastname . '.' . File::format($item->cv) ?>"><i class="fas fa-download"></i></a>
                    </div>
                    <div>
                        ...
                        <div class="sb-social">
                            <?php if ($item->linkedin) { ?>
                                <a href="<?= $item->linkedin ?>" target="_blank"><span
                                            class="icon-linkedin1"></span></a>
                            <?php } ?>
                            <?php if ($item->git_hub) { ?>
                                <a href="<?= $item->git_hub ?>" target="_blank"><span class="icon-github"></span></a>
                            <?php } ?>
                            <?php if ($item->stack_overflow) { ?>
                                <a href="<?= $item->stack_overflow ?>" target="_blank"><span
                                            class="icon-stackoverflow"></span></a>
                            <?php } ?>
                            <?php if ($item->site) { ?>
                ...
                <li>
                    ...
                    <form class="sb-form mar" id="question_form_<?= $item->id ?>">
                        ...
                        <div class="sb-row-sub">
                            <button class="btn-yellow"  onclick="load('portal/question/<?= $item->id ?>', 'form:#question_form_<?= $item->id ?>', 'vacancy=<?= $this->job->title ?>', 'vacancy_id=<?= $this->job->id ?>'); return false;" type="submit" >Send</button>
                        </div>
                    </form>
                </li>
                <li>
                    ...
                    <form class="sb-form" id="interview_form_<?= $item->id ?>">
                        ...
                        <div class="sb-row-sub">
                            <button class="btn-yellow" onclick="load('portal/interview/<?= $item->id ?>', 'form:#interview_form_<?= $item->id ?>', 'vacancy=<?= $this->job->title ?>', 'vacancy_id=<?= $this->job->id ?>'); return false;" type="submit">Send</button>
                        </div>
                    </form>
                </li>
                <li>
                    ...
                    <form class="sb-form" id="challenge_form_<?= $item->id ?>">
                        ...
                        <textarea class="sb-textarea" name="message" placeholder="Your Message"></textarea>
                        <div class="sb-row-sub">
                            <button class="btn-yellow" onclick="load('portal/challenge/<?= $item->id ?>', 'form:#challenge_form_<?= $item->id ?>', 'vacancy=<?= $this->job->title ?>', 'vacancy_id=<?= $this->job->id ?>'); return false;" type="submit">Send</button>
                        </div>
                    </form>
                </li>
                ...
                <?php } else { ?>
                    <li>
                        ...
                        <form class="sb-form" id="reject_form_<?= $item->id ?>">
                            ...
                            <textarea class="sb-textarea" name="message" placeholder="Your Message"></textarea>
                            <div class="sb-row-sub">
                                <button class="btn-yellow" onclick="load('portal/reject/<?= $item->id ?>', 'form:#reject_form_<?= $item->id ?>', 'vacancy=<?= $this->job->title ?>', 'vacancy_id=<?= $this->job->id ?>'); return false;" type="submit" >Send</button>
                            </div>
                        </form>
                    </li>
                <?php } ?>
...
<script>
    $(".link-status").click(function () {
        if (!$(this).hasClass('inactive-link') &&  $('.link-status').not($(this)).hasClass('inactive-link')) {
            $('.link-status').removeClass('inactive-link');
        } else {
            $('.link-status').addClass('inactive-link');
